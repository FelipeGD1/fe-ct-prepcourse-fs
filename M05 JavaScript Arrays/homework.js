...
function incrementarPorUno(array) {
   ...
   // Tu código:
   var incremetado= array.map((num)=>{return num+1});
   return incremetado;
}
...
function numeroMasGrande(arrayOfNums) {
   ...
   let numeroMayor;
    for(var i=0;i<arrayOfNums.length;i++ ){
        ...
        for(var j=1;j<arrayOfNums.length;j++){
            if(arrayOfNums[j]>arrayOfNums[i]){
             numeroMayor= arrayOfNums[j];
            }
       }
    }
    return numeroMayor;
...
function cuentoElementos(array) {
   ...
   // Tu código:
   var sumaDeNumerosMayoresA18=0;
   array.forEach(num => {
      if(num>18) sumaDeNumerosMayoresA18++} )
      return sumaDeNumerosMayoresA18;
   };
...
function empiezaConNueve(num) {
   ...
   var intArr= Array.from(string(num),func);
...
function breakStatement(num) {
   ...
   if(arregloo.length<10)return "Se interrumpio la ejecucion";
   return arregloo;
